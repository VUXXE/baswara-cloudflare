...
// Simple in-memory cache for fonts
...
export default async function handler(req: Request) {
  try {
    ...
    if (!cachedInterFont) {
      try {
        const fontResp = await fetch('https://raw.githubusercontent.com/rsms/inter/master/docs/font-files/Inter-Bold.woff');
        if (fontResp.ok) cachedInterFont = await fontResp.arrayBuffer();
      } catch (e) { console.error('Failed to fetch Inter font', e); }
    }
    
    if (!cachedSerifFont) {
      try {
        const fontResp = await fetch('https://raw.githubusercontent.com/fontsource/font-files/main/fonts/google/playfair-display/700-normal.woff');
        if (fontResp.ok) cachedSerifFont = await fontResp.arrayBuffer();
      } catch (e) { console.error('Failed to fetch Serif font', e); }
    }

    // Determine photo to show
    const showPhoto = photo && (photo.startsWith('http://') || photo.startsWith('https://'));
    ...
    // Construct line 1 and line 2 for description
      ...
      (
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', width: '48%' }}>
            ...
            {/* Badge - Wrapped in a flex row so it shrinks to fit content */}
            ...
            >
              ...
              <div style={{ display: 'flex' }}>{groom} & {bride}</div>
            </div>
            ...
            >
              {line1 ? <div style={{ display: 'flex' }}>{line1}</div> : null}
